...
class AllPhotoListView(ListView):
    model = Photo     
    template_name = 'photoapp/alllist.html'
...
class MyPhotoListView(ListView):
    ...
    def get_context_data(self,**kwargs):
        context = super().get_context_data(**kwargs)
        context['photos'] = Photo.objects.all().filter(submitter=self.request.user)
        print("context", context)
        return context


class PhotoTagListView(AllPhotoListView): 
...
class PhotoCreateView(LoginRequiredMixin, CreateView):
    model = Photo
    fields = ['title', 'description', 'image', 'tags', 'school', 'grade']
